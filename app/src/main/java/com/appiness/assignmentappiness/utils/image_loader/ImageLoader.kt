package com.appiness.assignmentappiness.image_loader
...
class ImageLoader (context: Context)  {
    ...
    init {
        ...
        executorService = Executors.newFixedThreadPool(5, ImageLoaderUtils.ImageThreadFactory())
        handler = Handler()
    ...
    private  fun loadImageIntoImageView(imageView: ImageView, bitmap: Bitmap?, imageUrl: String) {
        ...
        val scaledBitmap = ImageLoaderUtils.scaleBitmapForLoad(bitmap, imageView.width, imageView.height)
    ...
    inner class PhotosLoader(private var imageRequest: ImageRequest) : Runnable {
        ...
        override fun run() {
            ...
            val bitmap = ImageLoaderUtils.downloadBitmapFromURL(imageRequest.imgUrl)
